...
class GeneralController extends Controller implements HasMiddleware
{
  ...
  public static function middleware(): array
  {
    return [
      'auth',
      'verified',
      new Middleware('role:master-administrator', except: ['index', 'show', 'chart']),
    ];
  }
  ...
  public function __construct()
  {
    ...
    $this->middleware(function ($request, $next) {
      ...
      if ($user->hasRole('user') && in_array($action, ['index', 'show', 'chart'])) {
        return $next($request);
      } else if ($user->hasRole('master-administrator')) {
        return $next($request);
      } else {
        if (request()->ajax()) {
          return response()->json(['status' => 'error', 'message' => __('default.notification.error.restrict')], 403);
        }
